...
/**
 * @title StakedBNBToken
 *
 * @dev StakedBNBToken contract stores pool staked tokens.
 */
contract StakedBNBToken is IStakedBNBToken, OwnablePausableUpgradeable, ERC20PermitUpgradeable {
    ...
    // @dev Address of the RewardBNBToken contract.
    IRewardBNBToken private rewardBNBToken;
    ...
    /**
     * @dev See {IStakedBNBToken-toggleRewards}.
     */
    function toggleRewards(address account, bool isDisabled) external override onlyAdmin {
        require(account != address(0), "StakedBNBToken: invalid account");

        // toggle rewards
        rewardBNBToken.setRewardsDisabled(account, isDisabled);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override whenNotPaused {
        require(sender != address(0), "StakedBNBToken: invalid sender");
        require(recipient != address(0), "StakedBNBToken: invalid receiver");
        require(block.number > rewardBNBToken.lastUpdateBlockNumber(), "StakedBNBToken: cannot transfer during rewards update");

        // start calculating sender and recipient rewards with updated deposit amounts
        (bool senderRewardsDisabled, bool recipientRewardsDisabled) = rewardBNBToken.updateRewardCheckpoints(sender, recipient);
    ...
    /**
     * @dev See {IStakedBNBToken-mint}.
     */
    function mint(address account, uint256 amount) external override {
        require(msg.sender == pool, "StakedBNBToken: access denied");

        // start calculating account rewards with updated deposit amount
        bool rewardsDisabled = rewardBNBToken.updateRewardCheckpoint(account);
        if (rewardsDisabled) {
